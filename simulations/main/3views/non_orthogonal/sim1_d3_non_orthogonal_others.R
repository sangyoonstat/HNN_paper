...
seednum <- 2021 ; num_sim <- 130 ; jive_orth <- F
...
r0 <- 2 ; r1 <- 2 ; r2 <- 2 ; r3 <- 2 ; r23 <- 2 
theta <- c(30, 40, 50, 60) ; r12 <- r13 <- length(theta) ; snr <- 2
...
fit_others <- function(i, seednum, true_jive, jive_orth){
  ...
  Upairs <- gen_two_scores(n, theta) 
  #acos(sgv_single(crossprod(Upairs[[1]], Upairs[[2]]), tol))*(180/pi)
  U12_13 <- cbind(Upairs[[1]], Upairs[[2]])
  U23 <- scale(matrix(runif(n*r23), nrow = n, ncol = r23), scale = F)
  U23 <- qr.Q(qr((diag(1, n) - U12_13%*%solve(crossprod(U12_13))%*%t(U12_13))%*%U23))
  all_pairs <- list() ; all_pairs[[1]] <- Upairs[[1]] ; all_pairs[[2]] <- Upairs[[2]] ; all_pairs[[3]] <- U23
  
  allU <- gen_others_d3(r0, r1, r2, r3, all_pairs)
  ...
  dat <- dgen_d3_givenU(allU, pvec, sjoint, s12, s13, s23, s1ind, s2ind, s3ind, snr = rep(snr, length(pvec)), orthogonalV = T)
...
save(hnn_res_array, chosen_1se_vec, chosen_best_vec, chosen_min_vec, file = "./sim1_d3_non_orthogonal_others.RData")
