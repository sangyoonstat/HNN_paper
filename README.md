...
Two .R files to implement (i) HNN (.R file ending with HNN); (ii) other methods except AJIVE (.R file ending with others) for simulation studies with **orthogonal setup for two view case** in the main manuscript. The simulated data sets are within the subfolder named as "dat", which can be generated by the data generation part in either (i) or (ii) within corresponding folder.    

##### 1.3.1.2 2views/non-orthogonal
Two .R files to implement (i) HNN (.R file ending with HNN); (ii) other methods except AJIVE (.R file ending with others) for simulation studies with **non-orthogonal setup for two view case** in the main manuscript. The simulated data sets are within the subfolder named as "dat", which can be generated by the data generation part in either (i) or (ii) within corresponding folder.    

##### 1.3.1.3 3views/orthogonal
Two .R files to implement (i) HNN (.R file ending with HNN); (ii) other methods except AJIVE (.R file ending with others) for simulation studies with **orthogonal setup for three view case** in the main manuscript. The simulated data sets are within the subfolder named as "dat", which can be generated by the data generation part in either (i) or (ii) within corresponding folder.

##### 1.3.1.4 3views/non-orthogonal
Two .R files to implement (i) HNN (.R file ending with HNN); (ii) other methods except AJIVE (.R file ending with others) for simulation studies with **non-orthogonal setup for three view case** in the main manuscript. The simulated data sets are within the subfolder named as "dat", which can be generated by the data generation part in either (i) or (ii) within corresponding folder.
...
MATLAB code based on the original one [implemented in here](https://github.com/MeileiJiang/AJIVE_Project) to apply AJIVE for each simulation in the paper and appendix. Each .mat file within the "dat" subfolder can be loaded to reproduce the results.
